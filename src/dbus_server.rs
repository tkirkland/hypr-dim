...
impl DbusServer {
    pub fn new() -> Result<Option<Self>> {
        ...
        let gammarelay_root_iface = InterfaceImp::new("dev.hyprdim")
            ...
            .with_method::<(), ()>("Snapshot", snapshot_root_cb)
            .with_method::<(), ()>("Restore", restore_root_cb)
            .with_prop(
                "Inverted",
    ...
    pub fn add_output(&mut self, reg_name: u32, name: &str) {
        ...
        let gammarelay_output_iface = InterfaceImp::new("dev.hyprdim")
...
fn snapshot_root_cb(ctx: &mut MethodContext<WaylandState>, _args: ()) {
    ctx.state.snapshot_all();
    ctx.state.dimmed = true;
}

fn restore_root_cb(ctx: &mut MethodContext<WaylandState>, _args: ()) {
    ctx.state.restore_connected();
    ctx.state.dimmed = false;
}
...
fn signal_change(conn: &mut DuplexConn, path: &str, prop: &str, value: Param) {
    let output_sig = prop_changed_message(path, "dev.hyprdim", prop, value);
